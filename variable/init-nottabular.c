...
//  static double sqrt3 = sqrt(3.0);
 ...
 //   DOM_LOOP(k,j,i){};
  ...
  else if (side == X1_BEG)
  {
	...
	#ifdef VARIABLE_INFLOW
	//	B0 = g_tabularbfield[g_curidx] * MICROGAUSS;
        double zprime = ((init_dist - dist) - 1.5)/4.0;
        //Azimuthal field from Sun & Reich (2010) in cylindrical phi coordinate                                                                                                          
        double Bphi = 2.0/(1+zprime*zprime); 
        if (niter == 100000) {print1("Info: %f %f %f %f %f\n", 50.0-dist, g_tabulardistance[g_curidx], g_tabulardensity[g_curidx], g_tabularbfield[g_curidx], Bphi);niter=0;}
        niter++;
        B0 = Bphi * MICROGAUSS;
	#else
